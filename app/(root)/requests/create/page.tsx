...
import { createRequest } from "@/lib/networks/request";
import { CreateRequestType } from "@/lib/types/request";
import { Textarea } from "@/components/ui/textarea";
import { useSession } from "next-auth/react";
import SelectItemRequest from "@/components/Home/requests/SelectItemRequest";
...
export default function CreateRequest() {
  ...
  const { mutate: onCreateRequest, isPending } = useMutation({
  ...
  async function onSubmit(values: z.infer<typeof itemSchema>) {
    onCreateRequest({
      ...
      RequestItem: requestedItems.map((item) => ({
        itemId: item.id,
          ...
          <div className="flex gap-6">
            <div className="grid w-full flex-col flex-wrap gap-6 rounded-md bg-tertiary p-6 shadow-md lg:flex-row">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="">
                    <FormLabel>Reason of Item&lsquo;s Request</FormLabel>
                    <FormControl>
